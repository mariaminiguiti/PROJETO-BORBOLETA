// LoginPage.jsx - Página de login com autenticação Google Firebase
// Conceito shadcn: Card de login com glassmorphism e animações

'use client';

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import Image from 'next/image';
import Link from 'next/link';
import { signInWithGoogle } from '@/lib/firebase';

export default function LoginPage() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [isHovered, setIsHovered] = useState(false);
  const router = useRouter();

  const handleGoogleSignIn = async () => {
    setLoading(true);
    setError(null);

    const { user, error: signInError } = await signInWithGoogle();

    if (signInError) {
      setError(signInError);
      setLoading(false);
      return;
    }

    if (user) {
      // Redirect to home after successful login
      router.push('/');
    }

    setLoading(false);
  };

  const pageStyle = {
    minHeight: '100vh',
    background: 'linear-gradient(180deg, #f8f9fc 0%, #e8eaf0 50%, #f4f6ec 100%)',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    padding: '20px',
    position: 'relative',
    overflow: 'hidden',
  };

  const backgroundPatternStyle = {
    position: 'fixed',
    top: 0,
    left: 0,
    right: 0,
    bottom: 0,
    backgroundImage: `
      radial-gradient(circle at 20% 50%, rgba(255, 204, 0, 0.03) 0%, transparent 50%),
      radial-gradient(circle at 80% 80%, rgba(17, 0, 49, 0.05) 0%, transparent 50%)
    `,
    pointerEvents: 'none',
    zIndex: 0,
  };

  const containerStyle = {
    position: 'relative',
    zIndex: 1,
    maxWidth: '450px',
    width: '100%',
  };

  const cardStyle = {
    background: 'linear-gradient(135deg, #ffffff 0%, #fafafa 100%)',
    borderRadius: '24px',
    padding: '48px 40px',
    boxShadow: '0 20px 60px rgba(0, 0, 0, 0.12), 0 0 0 1px rgba(255, 204, 0, 0.1)',
    backdropFilter: 'blur(10px)',
    border: '1px solid rgba(0, 0, 0, 0.05)',
  };

  const logoContainerStyle = {
    textAlign: 'center',
    marginBottom: '32px',
  };

  const titleStyle = {
    fontSize: '2rem',
    fontWeight: '800',
    textAlign: 'center',
    marginBottom: '12px',
    background: 'linear-gradient(135deg, #110031 0%, #ffcc00 50%, #110031 100%)',
    WebkitBackgroundClip: 'text',
    WebkitTextFillColor: 'transparent',
    backgroundClip: 'text',
    letterSpacing: '0.5px',
  };

  const subtitleStyle = {
    textAlign: 'center',
    fontSize: '15px',
    color: '#666',
    marginBottom: '40px',
    lineHeight: '1.6',
  };

  const googleButtonStyle = {
    width: '100%',
    padding: '16px 24px',
    background: isHovered 
      ? 'linear-gradient(135deg, #ffffff 0%, #f9f9f9 100%)'
      : '#ffffff',
    border: isHovered 
      ? '2px solid rgba(255, 204, 0, 0.4)'
      : '2px solid #e0e0e0',
    borderRadius: '12px',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    gap: '12px',
    cursor: loading ? 'not-allowed' : 'pointer',
    transition: 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)',
    fontSize: '16px',
    fontWeight: '600',
    color: '#333',
    boxShadow: isHovered 
      ? '0 8px 24px rgba(255, 204, 0, 0.2)'
      : '0 4px 12px rgba(0, 0, 0, 0.08)',
    transform: isHovered ? 'translateY(-3px)' : 'translateY(0)',
    opacity: loading ? 0.6 : 1,
  };

  const errorStyle = {
    marginTop: '20px',
    padding: '14px',
    background: 'linear-gradient(135deg, rgba(220, 38, 38, 0.1), rgba(220, 38, 38, 0.05))',
    border: '1px solid rgba(220, 38, 38, 0.3)',
    borderRadius: '10px',
    color: '#991b1b',
    fontSize: '14px',
    textAlign: 'center',
  };

  const backLinkStyle = {
    display: 'block',
    textAlign: 'center',
    marginTop: '24px',
    color: '#666',
    textDecoration: 'none',
    fontSize: '14px',
    transition: 'color 0.3s ease',
  };

  const dividerStyle = {
    display: 'flex',
    alignItems: 'center',
    margin: '32px 0',
    color: '#999',
    fontSize: '14px',
  };

  const dividerLineStyle = {
    flex: 1,
    height: '1px',
    background: 'linear-gradient(to right, transparent, #e0e0e0, transparent)',
  };

  const featureListStyle = {
    marginTop: '32px',
    padding: '24px',
    background: 'linear-gradient(135deg, rgba(255, 204, 0, 0.05), rgba(17, 0, 49, 0.03))',
    borderRadius: '16px',
    border: '1px solid rgba(255, 204, 0, 0.1)',
  };

  const featureItemStyle = {
    display: 'flex',
    alignItems: 'center',
    gap: '12px',
    marginBottom: '12px',
    fontSize: '14px',
    color: '#555',
  };

  return (
    <div style={pageStyle}>
      <div style={backgroundPatternStyle} />
      <div style={containerStyle}>
        <div style={cardStyle}>
          <div style={logoContainerStyle}>
            <Image 
              src="/eventuslogobg.png" 
              alt="Eventus Logo" 
              width={180} 
              height={180}
              style={{ filter: 'drop-shadow(0 0 20px rgba(255, 204, 0, 0.3))' }}
            />
          </div>

          <h1 style={titleStyle}>Bem-vindo de volta! 🎭</h1>
          <p style={subtitleStyle}>
            Faça login para acessar sua agenda cultural personalizada
          </p>

          <button
            onClick={handleGoogleSignIn}
            disabled={loading}
            style={googleButtonStyle}
            onMouseEnter={() => !loading && setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
          >
            {loading ? (
              <>
                <div style={{
                  width: '20px',
                  height: '20px',
                  border: '3px solid rgba(255, 204, 0, 0.2)',
                  borderTop: '3px solid #ffcc00',
                  borderRadius: '50%',
                  animation: 'spin 1s linear infinite',
                }} />
                <span>Conectando...</span>
              </>
            ) : (
              <>
                <svg width="24" height="24" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                <span>Continuar com Google</span>
              </>
            )}
          </button>

          {error && (
            <div style={errorStyle}>
              ⚠️ {error}
            </div>
          )}

          <div style={dividerStyle}>
            <div style={dividerLineStyle} />
            <span style={{ margin: '0 16px' }}>ou</span>
            <div style={dividerLineStyle} />
          </div>

          <div style={featureListStyle}>
            <div style={featureItemStyle}>
              <span style={{ fontSize: '20px' }}>✨</span>
              <span>Acesso a eventos exclusivos</span>
            </div>
            <div style={featureItemStyle}>
              <span style={{ fontSize: '20px' }}>🎫</span>
              <span>Salve seus eventos favoritos</span>
            </div>
            <div style={featureItemStyle}>
              <span style={{ fontSize: '20px' }}>🔔</span>
              <span>Receba notificações personalizadas</span>
            </div>
            <div style={featureItemStyle}>
              <span style={{ fontSize: '20px' }}>🎨</span>
              <span>Crie sua agenda cultural</span>
            </div>
          </div>

          <Link 
            href="/" 
            style={backLinkStyle}
            onMouseEnter={(e) => e.currentTarget.style.color = '#ffcc00'}
            onMouseLeave={(e) => e.currentTarget.style.color = '#666'}
          >
            ← Voltar para página inicial
          </Link>
        </div>
      </div>

      <style jsx>{`
        @keyframes spin {
          0% { transform: rotate(0deg); }
          100% { transform: rotate(360deg); }
        }
      `}</style>
    </div>
  );
}
